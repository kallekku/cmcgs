...
parser.add_argument('--cmcgs', action='store_true', help='CMCGS')
parser.add_argument('--greedy-action', action='store_true', help='CMCGS: Do not select the greedy action')
parser.add_argument('--optimal-ratio', type=float, default=0.3, help='CMCGS: Ratio of optimal rollouts')
parser.add_argument('--interactions', type=int, default=120000, help='CMCGS: Number of interactions')
parser.add_argument('--max-clusters', type=int, default=np.inf, help='CMCGS: Max clusters')
parser.add_argument('--clustering', type=str, default='agglomerative', help='CMCGS: Clustering alg')
parser.add_argument('--linkage', type=str, default='ward', help='CMCGS: Linkage for agglomerative clustering')
...
if args.models != '' and os.path.exists(args.models):
    model_dicts = torch.load(args.models, map_location=device)
    transition_model.load_state_dict(model_dicts['transition_model'])
    observation_model.load_state_dict(model_dicts['observation_model'])
    reward_model.load_state_dict(model_dicts['reward_model'])
    encoder.load_state_dict(model_dicts['encoder'])
    optimiser.load_state_dict(model_dicts['optimiser'])
planner = MPCPlanner(env.action_size, args.planning_horizon, args.optimisation_iters, args.candidates, args.top_candidates, transition_model, reward_model, args.state_size, env.action_range[0], env.action_range[1], args.cmcgs, args.clustering, args.greedy_action, args.optimal_ratio, args.max_clusters, args.interactions, args.linkage)
...
if args.test:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(args.test_episodes)):
            ...
            #pbar = range(args.max_episode_length // args.action_repeat)
            for t in pbar:
