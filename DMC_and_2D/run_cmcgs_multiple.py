...
def main(config):
    ...
    with Pool(10) as p:
        p.map(eval_cmcgs, [(config, r, False) for r in range(runs)])

def eval_cmcgs(args):
    config, seed, verbose = args
    random.seed(seed)
    np.random.seed(seed)
    
    budget = get_budget_string(config.agent)
    file_name = f"{config.agent.name}_{budget}_{config.env_name}_{seed}"
    resdir = "results-csv"

    print('env_name: %s\nsave_video: %d\nuse_wandb: %d\nagent: %s\nrun_suffix: %s'
          % (config.env_name, config.save_video, config.use_wandb, config.agent, seed))
    ...
    if config.agent.name == "cmcgs" and verbose:
        agent.report_time_budget()

    if not config.use_wandb:
        log_dir = os.path.join(os.getcwd(), resdir) 
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        #agent_name = config.agent.name
        #budget = get_budget_string(config.agent)
        #log_dir = os.path.join(log_dir, "%s_%s_%s_%s.csv" % (agent_name, budget, config.env_name, seed))
        log_dir = os.path.join(log_dir, "%s.csv" % (file_name))
